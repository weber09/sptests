...
public class AlgolParser implements AlgolParserConstants {
    ...
    static public SPCompilationUnit compilationUnit() throws ParseException {
        ...
        fileName = String.format("sp_algol_%d%d%d%d%d", LocalDateTime.now().getDayOfMonth(), LocalDateTime.now().getMonthValue(), LocalDateTime.now().getYear(), LocalDateTime.now().getHour(), LocalDateTime.now().getMinute());
    ...
    static public ArrayList<SPVariableDeclarator> VariableInitializer(ArrayList<SPStatement> arrayInitializers) throws ParseException {
    ...
    static public SPStatement Statement() throws ParseException {
    ...
    static public SPExpression ConditionalOrExpression() throws ParseException {
        ...
        while (true) {
            ...
            SPExpression rhs = ConditionalAndExpression();//TODO: fazer funcinar
    ...
    static public SPExpression ConditionalAndExpression() throws ParseException {
        ...
        while (true) {
            ...
            SPExpression rhs = EqualityExpression();//TODO: fazer funcinar
    ...
    static public SPExpression Selector(SPExpression target) throws ParseException {
        ...
        switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
            case LBRACKET:
                ...
                ArrayList<SPExpression> indexExpressions = new ArrayList<>();

                SPExpression indexExpression = Expression();

                indexExpressions.add(indexExpression);
                boolean multiArrayContinue = true;
                while (multiArrayContinue) {
                    switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
                        case COMMA:
                            jj_consume_token(COMMA);
                            ...
                            break;
                        default:
                            multiArrayContinue = false;
                            break;
                    }
                }
                expr = new SPArrayExpression(line, target, indexExpressions);
                jj_consume_token(RBRACKET);
                break;
            default:
                jj_la1[44] = jj_gen;
    ...
    static public SPStatement WriteStatement() throws ParseException {
        ...
        switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
            ...
            case IDENTIFIER: {
                switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
                    case IDENTIFIER: {
                        SPExpression param = Expression();
                        printParams.add(param);
                        break;
                    }
                    case VERDADEIRO: {
                        jj_consume_token(VERDADEIRO);
                        SPExpression param = new SPLiteralTrue(token.beginLine);
                        break;
                    }
                    case FALSO: {
                        jj_consume_token(FALSO);
                        SPExpression param = new SPLiteralFalse(token.beginLine);
                        break;
                    }
                    case INTEGER_LITERAL:
                    case FLOATING_POINT_LITERAL:
